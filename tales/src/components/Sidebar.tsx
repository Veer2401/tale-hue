...
export default function Sidebar({ onNavigate, currentView }: SidebarProps) {
  ...
  return (
    <div className="fixed left-0 top-0 h-screen w-64 glass-heavy border-r border-white/20 flex flex-col backdrop-blur-xl bg-gradient-to-b from-zinc-900/98 via-zinc-900/95 to-zinc-950/98">
      {/* Logo */}
      ...
      <nav className="flex-1 p-4 space-y-3">
        {menuItems.map((item) => (
          <button
            ...
            className={`group w-full flex items-center gap-4 px-5 py-4 rounded-2xl transition-all duration-300 shimmer-hover ${currentView === item.view
                ? 'bg-gradient-to-r from-purple-500 via-pink-600 to-orange-500 text-white shadow-lg shadow-purple-400/50 neon-glow-strong button-glow'
                : 'glass-light hover:glass text-zinc-300 hover:text-white border border-white/10 hover:border-purple-400/60 button-glow'
              }`}
          >
            <item.icon size={24} />
        ...
        <div className="pt-4 space-y-3">
          {user && profile ? (
            <div className="px-4 py-4 rounded-2xl glass-gradient border-2 border-purple-400/40 backdrop-blur-sm">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-purple-400 via-pink-500 to-orange-500 flex items-center justify-center text-white font-bold text-lg shadow-lg ring-2 ring-white/20 pulse-glow">
              ...
              <button
                onClick={signOut}
                className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-red-500/20 hover:bg-red-500/30 border border-red-500/50 text-red-300 hover:text-white text-sm font-bold transition-all duration-300 button-glow"
              >
                <LogOut size={18} />
                Sign Out
              </button>
            ...
            <div className="space-y-3">
              {error && (
                <div className="p-3 bg-red-500/10 border border-red-500/40 rounded-xl backdrop-blur-sm fade-in">
              ...
              <button
                ...
                disabled={signingIn}
                className="w-full flex items-center justify-center gap-3 px-5 py-4 rounded-2xl bg-orange-900 hover:bg-orange-800 text-white font-bold shadow-lg shadow-orange-900/50 hover:shadow-orange-800/60 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed button-glow shimmer-hover"
              >
                {signingIn ? (
                  <>
